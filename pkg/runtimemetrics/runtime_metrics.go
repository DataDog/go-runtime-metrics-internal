...
type Options struct {
	...
	// TagPeriod is the period at which we refresh tags like gogc, gomemlimit,
	// etc.. Defaults to 5 minutes to avoid excessive tag churn for applications
	// using dynamic runtime tuning.
	TagPeriod time.Duration
...
func NewEmitter(statsd partialStatsdClientInterface, opts *Options) (*Emitter, error) {
	...
	tagPeriod := cmp.Or(opts.TagPeriod, 5*time.Minute)
	baseTags := newTagCacher(tagPeriod, time.Now, func() []string {
		return append(getBaseTags(), opts.Tags...)
	})
	e := &Emitter{
		statsd:   statsd,
...
func (e *Emitter) emit() {
	...
	rms := newRuntimeMetricStore(descs, e.statsd, e.logger, e.baseTags)
...
func newRuntimeMetricStore(descs []metrics.Description, statsdClient partialStatsdClientInterface, logger *slog.Logger, baseTags func() []string) runtimeMetricStore {
	if baseTags == nil {
		baseTags = func() []string { return nil }
	}
